...
class ae {
  ...
  intersects(e, t, o) {
    const s = (b, k, C, I, R, B) => {
      ...
      return b >= O && b <= K && k >= Y && k <= T;
    }, l = t.x, u = t.y, h = o.x, p = o.y, g = (p - u) / (h - l), S = u - g * l, f = (b, k) => {
      const C = this.x_points[b], I = this.y_points[b] + e, R = this.x_points[k], B = this.y_points[k] + e, O = (B - I) / (R - C), K = I - O * C;
      if (O == g)
...
class Q {
  ...
  registerRegion(e, t) {
    const o = /* @__PURE__ */ new Map(), s = (k, C) => {
      const I = o.get(C.pointerId);
      I !== void 0 && I.moved && (k == "touch" ? t.dragCancel && t.dragCancel() : t.penDragCancel && t.penDragCancel()), o.delete(C.pointerId);
    }, l = (k, C) => (I) => {
      if (I.pointerType != k)
        return;
      ...
      o.set(C.pointerId, new fn(I)), o.size > 1 && s(k, C);
      const R = k === "touch" ? t.longPress : t.penLongPress;
      R && setTimeout(async () => {
        const B = o.get(C.pointerId);
        B === void 0 || B.moved || await R(B.initialPos) && s(k, C);
      }, hn);
    }, g = (k, C) => {
      const I = o.get(C.pointerId);
      if (I === void 0)
      ...
      I.moved ? k == "touch" ? t.dragEnd && t.dragEnd(R) : t.penDragEnd && t.penDragEnd(R) : k == "touch" ? t.tap && t.tap(I.initialPos) : t.penTap && t.penTap(I.initialPos), s(k, C);
    }, S = (k, C) => {
      const I = o.get(C.pointerId);
      if (I === void 0)
        return;
      const R = Q.getCanvasCoords(C), B = R.x - I.lastPos.x, O = R.y - I.lastPos.y, K = Math.sqrt(B * B + O * O), Y = k === "touch" ? this.touchMoveThreshold : this.penMoveThreshold;
      if (I.moved || K > Y) {
        let T = I.lastPos;
  ...
  installEventHandlers() {
    ...
    if (s.registerRegion(this.ctx.canvas, g), !this.readonly) {
      const S = {
        ...
        longPress: (k) => {
          navigator.vibrate([100]), o = "indent", t = Math.floor(this.transformCoords(k).y / this.line_spacing);
        }
      };
...
async function xn(E, e, t) {
  const o = new te("Quick Links"), s = E.rootOnlyDoc(), l = o.add_canvas(), u = new Ge(l, s.doc, e, !0);
  ...
  t.onclick = async () => {
    ...
    for (let p of h) {
      ...
      b.innerText = "Delete?", b.classList.add("modalalert-cancel"), b.style.float = "none", b.style.marginTop = "0", p === E && (b.disabled = !0), g.appendChild(b), g.appendChild(document.createElement("br")), g.appendChild(document.createElement("br")), b.onclick = async () => {
        await yn(`Delete ${p}?`) && (await e.deleteNotebook(p), await Ze(E, e)), o.close_container(), await new Promise((C) => {
          setTimeout(C, 0);
        }), t.click();
...
async function Cn(E) {
  const e = document.getElementById("style-dark").innerText, t = document.getElementById("style-light").innerText, o = document.getElementById("style-current"), s = (u) => {
    G.setItem("theme", u ? "dark" : "light"), o.innerText = u ? e : t, E.clearAndRedraw();
  };
  ...
  l || (l = window.matchMedia("(prefers-color-scheme: light)").matches ? "light" : "dark"), l == "light" ? j().enableLightMode() : j().enableDarkMode();
}
function Ln(E, e, t) {
...
function In(E) {
  ...
  t.onclick = async () => {
    const s = await E.engine.pop();
    s && (await E.clearAndRedraw(), e.push(s));
  };
  const o = document.getElementById("Menubar.Edit.Redo");
  o.onclick = async () => {
    const s = e.pop();
    s && (await E.engine.execute(s), await E.clearAndRedraw());
  };
}
function Rn(E, e) {
  ...
  o.onclick = async () => {
    await E.add_line(e, Math.floor(Number(s.value))), h();
  }, l.onclick = () => {
...
async function Nn() {
  ...
  await h.setActiveNotebook(u), await h.notebookIsInitialized() ? (await g.load(s), g.clearAndRedraw()) : (await g.save(), await h.initializeNotebook()), g.on_line_select = Rn.bind(null, g), g.installEventHandlers(), En(), kn(u, h), await Ze(u, h), await Cn(g), Ln(u, h, g), Sn(g), In(g);
